		...
		<!-- {if $data.article_related_goods} -->
			...
			<div class="article-content clearfix goods-list"> 
				<div class="article-content-sku clearfix" data-lazy="false"> 
					<a class="nopjax external" href='{url path="goods/index/show"}&goods_id={$goods.goods_id}{if $goods.product_id neq 0}&product_id={$goods.product_id}'><img class="content-sku-img lazy-img" src="{$goods.img.thumb}"></a>
					<div class="content-sku-right"> 
						<a class="nopjax external" href='{url path="goods/index/show"}&goods_id={$goods.goods_id}{if $goods.product_id neq 0}&product_id={$goods.product_id}'><p class="content-sku-name line-clamp2">{$goods.name}</p></a>
						<div class="content-sku-bottom clearfix"> 
							<span class="sku-price ellipsis">{$goods.shop_price}</span>  
							<a class="nopjax external" href='{url path="goods/index/show"}&goods_id={$goods.goods_id}{if $goods.product_id neq 0}&product_id={$goods.product_id}'><span class="buy-btn">{t domain="h5"}去购买{/t}</span></a>
						</div> 
					</div> 
				</div>  
			</div>
            ...
            <div class="form-group ecjia-form ecjia-like-goods-list">
                <div class="bd">
					<ul class="ecjia-list ecjia-like-goods-list">
						...
						<li>
							<a class="nopjax external" href='{url path="goods/index/show" args="goods_id={$goods.goods_id}{if $goods.product_id neq 0}&product_id={$goods.product_id}'>
