...
class affiliate_controller
{
    ...
    public static function store_affiliate()
    {
    	...
    	if (!ecjia_front::$controller->is_cached('store_affiliate.dwt', $cache_id)) {
    		ecjia_front::$controller->assign_title(__('推广店铺入驻', 'h5'));
    		
    		$invite_store_detail = ecjia_touch_manager::make()->api(ecjia_touch_api::INVITE_STORE)->data(array('token' => $token))->run();
    		$invite_store_detail = is_ecjia_error($invite_store_detail) ? array() : $invite_store_detail;
    		ecjia_front::$controller->assign('invite_store', $invite_store_detail);
    		...
    		if (user_function::is_weixin()) {
    			$spread_url = RC_Uri::url('affiliate/index/store_affiliate');
    			$config     = user_function::get_wechat_config($spread_url);
    			ecjia_front::$controller->assign('config', $config);
    		}
    	}
    	return ecjia_front::$controller->display('store_affiliate.dwt', $cache_id);
    }
    ...
    public static function store_agent()
    {
    	...
    	if (!ecjia_front::$controller->is_cached('store_agent.dwt', $cache_id)) {
    		...
    		$invite_agent_detail = ecjia_touch_manager::make()->api(ecjia_touch_api::INVITE_STORE_AGENT_USER)->data(array('token' => $token))->run();
    		$invite_agent_detail = is_ecjia_error($invite_agent_detail) ? array() : $invite_agent_detail;
    		$invite_agent_detail['invite_url'] = RC_Uri::url('affiliate/index/invite_agent', array('invite_code' => $invite_agent_detail['invite_code']));
    		ecjia_front::$controller->assign('invite_agent', $invite_agent_detail);
    	...
    	return ecjia_front::$controller->display('store_agent.dwt', $cache_id);
    }
     ...
     * 二级代理填写信息
     */
    public static function invite_agent() 
    {
    	...
    	$agent_info = is_ecjia_error($agent_info) ? [] : $agent_info;
    	ecjia_front::$controller->assign('agent_info', $agent_info);
  
    	return ecjia_front::$controller->display('affiliate_invite_agent.dwt');
    }
    
    /**
     * 二级代理填写信息
     */
    public static function invite_agent_insert()
    {
    	$agent_name = trim($_POST['agent_name']);
    	$invite_code = intval($_POST['invite_code']);
    	if (empty($agent_name)) {
    		return ecjia_front::$controller->showmessage(__('请输入代理商名称', 'h5'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
    	}
    	...
    	$res = ecjia_touch_manager::make()->api(ecjia_touch_api::INVITE_STORE_AGENT_JOIN)->data($param)->run();
    	if (is_ecjia_error($res)) {
    		return ecjia_front::$controller->showmessage($res->get_error_message(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
    	}
    	return ecjia_front::$controller->showmessage(__('加入二级代理商成功', 'h5'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('touch/index/init')));
    }
}
