...
class affiliate_controller
{
    ...
    public static function check()
    {
        ...
        if ($res['registered'] == 1) {
            return ecjia_front::$controller->showmessage(__('您已经是老用户，可以直接去登陆'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('registered' => 1, 'url' => RC_Uri::url('user/privilege/login')));
        }

        if ($res['is_invited'] == 1) {
            return ecjia_front::$controller->showmessage(__('该手机号已被推荐邀请'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }

        return ecjia_front::$controller->showmessage(__('验证码已发送'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
    }
    ...
    public static function invite()
    {
        ...
        if (empty($code)) {
            return ecjia_front::$controller->showmessage(__('请输入短信验证码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        return ecjia_front::$controller->showmessage(__('领取成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('touch/index/init')));
    }
}
