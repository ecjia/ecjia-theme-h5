...
class cart_controller
{
    ...
    public static function init()
    {
        ...
        ecjia_front::$controller->assign_title(__('购物车列表'));
    ...
    public static function checkout()
    {
        ...
        if (!ecjia_touch_user::singleton()->isSignin()) {
            return ecjia_front::$controller->showmessage(__('请先登录再继续操作'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('cart/index/init')));
        }
        ...
        if (empty($rec_id)) {
            return ecjia_front::$controller->showmessage(__('请选择商品再进行结算'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('cart/index/init')));
        }
        ...
        if ($_POST['payment_shipping_update']) {
            ...
            if (empty($shipping_id)) {
                return ecjia_front::$controller->showmessage(__('请选择配送方式'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        } else {
        ...
        if ($_POST['inv_update']) {
            if (empty($_POST['inv_content']) || $_POST['inv_type_name'] == 'enterprise') {
                if (empty($_POST['inv_payee']) || empty($_POST['inv_bill_code'])) {
                    return ecjia_front::$controller->showmessage(__('请填写完整的发票信息'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
                }
            }
        ...
        if ($_POST['integral_update']) {
            if ($_POST['integral'] > $_SESSION['cart'][$cart_key]['data']['order_max_integral']) {
                return ecjia_front::$controller->showmessage(__(sprintf("%s使用超出订单限制", $integral_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            } else if ($_POST['integral'] > $_SESSION['cart'][$cart_key]['data']['your_integral']) {
                return ecjia_front::$controller->showmessage(__(sprintf("%s不足", $integral_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            } else {
        ...
        ecjia_front::$controller->assign_title(__('结算'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function storepickup_checkout()
    {
        ...
        $integral_name = !empty(ecjia::config('integral_name')) ? ecjia::config('integral_name') : __('积分');
        //积分
        if ($_POST['integral_update']) {
            if ($_POST['integral'] > $_SESSION['cart'][$cart_key]['data']['order_max_integral']) {
                return ecjia_front::$controller->showmessage(__(sprintf("%s使用超出订单限制", $integral_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
...
//        $total['shipping_fee']          = $selected_shipping['shipping_fee']; //$rs['shipping_list'];
    ...
    public static function goods_list()
    {
        ...
        if (empty($address_id)) {
            ...
            return ecjia_front::$controller->showmessage(__('请先选择收货地址'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        }
        ...
        ecjia_front::$controller->assign_title(__('商品清单'));
    ...
    public static function done()
    {
        ...
        if (empty($rec_id)) {
            return ecjia_front::$controller->showmessage(__('请选择商品再进行结算'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }

        if (empty($address_id)) {
            $pjax_url = !empty($_SESSION['order_address_temp']['pjax_url']) ? trim($_SESSION['order_address_temp']['pjax_url']) : RC_Uri::url('cart/index/init');
            return ecjia_front::$controller->showmessage(__('请先选择收货地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        }
        ...
        if (empty($shipping_id)) {
            return ecjia_front::$controller->showmessage(__('当前收货地址暂无可用配送方式，请重新更换其他的收货地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }

        if (empty($pay_id)) {
            return ecjia_front::$controller->showmessage(__('请选择一个支付方式'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if (empty($support_cod) && $pay_code == 'pay_cod') {
            return ecjia_front::$controller->showmessage(__('当前配送方式不支持货到付款'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    public static function storepickup_done()
    {
        ...
        if (empty($pickup_date) || empty($pickup_time)) {
            return ecjia_front::$controller->showmessage(__('请选择提货时间'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    {
        if (!ecjia_touch_user::singleton()->isSignin()) {
            return ecjia_front::$controller->showmessage(__('请先登录再继续操作'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('cart/index/init')));
        }
        $address_id = empty($_GET['address_id']) ? 0 : intval($_GET['address_id']);
        $rec_id     = empty($_GET['rec_id']) ? 0 : trim($_GET['rec_id']);
        ...
        $url = RC_Uri::site_url() . substr($_SERVER['REQUEST_URI'], strripos($_SERVER['REQUEST_URI'], '/'));
        if (empty($rec_id)) {
            return ecjia_front::$controller->showmessage(__('请选择商品再进行结算'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('cart/index/init')));
        }
        if (empty($address_id)) {
            $pjax_url = !empty($_SESSION['order_address_temp']['pjax_url']) ? trim($_SESSION['order_address_temp']['pjax_url']) : RC_Uri::url('cart/index/init');
            return ecjia_front::$controller->showmessage(__('请先选择收货地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
    ...
    public static function shipping()
    {
        ...
        ecjia_front::$controller->assign('address_id', $address_id);
        ecjia_front::$controller->assign('rec_id', $rec_id);
        ecjia_front::$controller->assign_title(__('配送方式'));
    ...
    public static function pay_shipping()
    {
        ...
        ecjia_front::$controller->assign_title(__('选择支付配送方式'));
    ...
    public static function shipping_date()
    {
        ...
        if (empty($rec_id)) {
            return ecjia_front::$controller->showmessage(__('请选择商品再进行结算'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
        }
        if (empty($address_id)) {
            $pjax_url = !empty($_SESSION['order_address_temp']['pjax_url']) ? trim($_SESSION['order_address_temp']['pjax_url']) : RC_Uri::url('cart/index/init');
            return ecjia_front::$controller->showmessage(__('请先选择收货地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        ...
        ecjia_front::$controller->assign_title(__('配送时间'));
    ...
    public static function invoice()
    {
        ...
        ecjia_front::$controller->assign_title(__('开发票'));
    ...
    public static function note()
    {
        ...
        ecjia_front::$controller->assign_title(__('备注留言'));
    ...
    public static function bonus()
    {
        ...
        if ($data['allow_use_bonus'] == 0) {
            return ecjia_front::$controller->showmessage(__('红包不可用'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
        }
        ...
        ecjia_front::$controller->assign_title(__('使用红包'));
    ...
    public static function integral()
    {
        ...
        if ($data['order_max_integral'] == 0) {
            return ecjia_front::$controller->showmessage(__(sprintf("%s不可用", $integral_name)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
        }
        ecjia_front::$controller->assign('data', $data);
        ecjia_front::$controller->assign('temp', $temp);
        ecjia_front::$controller->assign('address_id', $address_id);
        ...
        ecjia_front::$controller->assign_title(__(sprintf("使用%s", $integral_name)));
