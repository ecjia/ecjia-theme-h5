...
class connect_controller
{
    ...
    public static function callback_template($data)
    {
        ...
        ecjia_front::$controller->assign('title', __('绑定手机号'));
        ecjia_front::$controller->assign_title(__('绑定手机号'));
    ...
    public static function bind_signup($params)
    {
        ...
        if (!$params) {
            if (empty($connect_code) || empty($open_id)) {
                return ecjia_front::$controller->showmessage(__('授权信息异常，请重新授权'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR);
            }
            ecjia_front::$controller->assign('connect_code', $connect_code);
            ecjia_front::$controller->assign('open_id', $open_id);

            ecjia_front::$controller->assign('title', __("注册绑定"));
            ecjia_front::$controller->assign_title(__("注册绑定"));
    ...
    public static function bind_signup_do()
    {
        ...
        if (empty($mobile) || empty($username) || empty($password) || empty($code)) {
            return ecjia_front::$controller->showmessage(__('请填写完整信息'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($connect_code) || empty($open_id)) {
            return ecjia_front::$controller->showmessage(__('授权信息异常，请重新授权'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if ($result) {
            ...
            return ecjia_front::$controller->showmessage(__('恭喜您，注册成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $return_url));
        } else {
            return ecjia_front::$controller->showmessage(__('授权用户信息关联失败'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    }
    ...
    public static function bind_signin()
    {
        ...
        ecjia_front::$controller->assign('title', __("验证并关联"));
        ecjia_front::$controller->assign_title(__("验证并关联"));
    ...
    public static function bind_signin_do()
    {
        ...
        if (is_ecjia_error($data)) {
            ...
            if ($data['id']) {
                ...
                return ecjia_front::$controller->showmessage(__('用户验证成功，获取用户信息失败，请重试！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if ($result) {
                ...
                return ecjia_front::$controller->showmessage(__('关联成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS,
    ...
    public static function mobile_login()
    {
        ...
        if (empty($mobile_phone)) {
            return ecjia_front::$controller->showmessage(__('请输入手机号'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    public static function captcha_validate()
    {
        ...
        ecjia_front::$controller->assign('title', __('图形验证码'));
        ecjia_front::$controller->assign_title(__('图形验证码'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function captcha_check()
    {
        ...
        if (RC_Time::gmtime() < $_SESSION['user_temp']['resend_sms_time'] + 60) {
            return ecjia_front::$controller->showmessage(__('规定时间1分钟以外，可重新发送验证码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        $param = array(
            'token'        => $token,
            'type'         => 'mobile',
    ...
    public static function mobile_signin()
    {
        ...
        if ($registered == 1) {
            ...
            if ($result) {
                return ecjia_front::$controller->showmessage(__('关联成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => $url));
            } else {
                return ecjia_front::$controller->showmessage(__('授权用户信息关联失败'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        } else {
    ...
    public static function set_password()
    {
        ...
        if (isset($_POST['username'])) {
            ...
            if (empty($username)) {
                return ecjia_front::$controller->showmessage(__('请设置用户名'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if (empty($password)) {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            if (!ecjia_front::$controller->is_cached('user_set_password.dwt', $cache_id)) {
                ecjia_front::$controller->assign('title', __('设置名字密码'));
                ecjia_front::$controller->assign_lang();
            }
