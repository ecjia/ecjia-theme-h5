...
class franchisee_controller
{
    ...
    public static function first()
    {
        ...
        if ($config['merchant_join_close'] == 1) {
            return ecjia_front::$controller->showmessage(__('抱歉，该网站已关闭入驻商加盟！'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR);
        }
        ...
        ecjia_front::$controller->assign_title(__('店铺入驻'));
    ...
    public static function first_check()
    {
        ...
        if (empty($name)) {
            return ecjia_front::$controller->showmessage(__('请输入真实姓名'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($email)) {
            return ecjia_front::$controller->showmessage(__('请输入电子邮箱'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($mobile)) {
    ...
    public static function second()
    {
        ...
        ecjia_front::$controller->assign_lang();
        ecjia_front::$controller->assign_title(__('进度查询'));
    ...
    public static function second_check()
    {
        ...
        if (is_ecjia_error($rs)) {
            ...
            return ecjia_front::$controller->showmessage(__('验证码已发送'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('franchisee/index/three', array('mobile' => $mobile, 'code' => $code))));
        }
    }
    ...
    public static function three()
    {
        ...
        ecjia_front::$controller->assign('title', __('输入验证码'));
        ecjia_front::$controller->assign_title(__('输入验证码'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function three_check()
    {
        ...
        if (is_ecjia_error($rs)) {
            ...
            return ecjia_front::$controller->showmessage(__('校验成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('franchisee/index/four')));
        }
    }
    ...
    public static function resend_sms()
    {
        ...
        if (is_ecjia_error($rs)) {
            ...
            return ecjia_front::$controller->showmessage(__('发送成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
        }
    }
    ...
    public static function send_sms()
    {
        ...
        if (!empty($mobile)) {
            ...
            if (is_ecjia_error($rs)) {
                ...
                return ecjia_front::$controller->showmessage(__(sprintf("短信已发送到手机%s，请注意查看", $mobile)), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
            }
        } else {
            if (empty($mobile)) {
                return ecjia_front::$controller->showmessage(__('请输入手机号码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
    ...
    public static function four()
    {
        ...
        if ($config['merchant_join_close'] == 1) {
            return ecjia_front::$controller->showmessage(__('抱歉，该网站已关闭入驻商加盟！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        //验证第一步是否通过
        if (empty($_SESSION['franchisee_add']) || $_SESSION['franchisee_add']['access_time'] + 1800 < RC_Time::gmtime()) {
            return ecjia_front::$controller->showmessage(__('请先填写基本信息'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('franchisee/index/first')));
        }
        ...
        if (is_ecjia_error($mobile)) {
            ...
            if ($reaudit['validate_type'] == 2) {
                $validate_type = __('企业入驻');
            } else {
                $validate_type = __('个人入驻');
            }
    ...
    public static function finish()
    {
        ...
        if (empty($email)) {
            return ecjia_front::$controller->showmessage(__('请输入邮箱地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($mobile)) {
            return ecjia_front::$controller->showmessage(__('请输入手机号码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($seller_name)) {
            return ecjia_front::$controller->showmessage(__('请输入商店名称'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (mb_strlen($seller_name) > 17) {
            return ecjia_front::$controller->showmessage(__('店铺名称不能超过17个字符'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($seller_category)) {
            return ecjia_front::$controller->showmessage(__('请选择店铺分类'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($validate_type)) {
            return ecjia_front::$controller->showmessage(__('请选择店铺类型'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($province)) {
            return ecjia_front::$controller->showmessage(__('请选择省'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($city)) {
            return ecjia_front::$controller->showmessage(__('请选择市'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($district)) {
            return ecjia_front::$controller->showmessage(__('请选择区'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($street)) {
            return ecjia_front::$controller->showmessage(__('请选择街道'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($address)) {
            return ecjia_front::$controller->showmessage(__('请填写详细地址'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if (is_ecjia_error($data)) {
            ...
            return ecjia_front::$controller->showmessage(__("入驻信息已提交，请耐心等耐审核"), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('franchisee/index/process', 'show=1')));
        }
    }
    ...
    public static function process_captcha_check()
    {
        ...
        if (is_ecjia_error($rs)) {
            ...
            return ecjia_front::$controller->showmessage(__('验证码已发送'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('franchisee/index/process_enter_code', array('mobile' => $mobile, 'code' => $code))));
        }
    }
    ...
    public static function process_validate_code()
    {
        ...
        if (is_ecjia_error($rs)) {
            ...
            return ecjia_front::$controller->showmessage(__('校验成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('franchisee/index/process', array('mobile' => $mobile, 'code' => $code))));
        }
    }
    ...
    public static function process()
    {
        ...
        ecjia_front::$controller->assign_lang();
        ecjia_front::$controller->assign_title(__('申请进度'));
    ...
    public static function location()
    {
        ...
        ecjia_front::$controller->assign_lang();
        ecjia_front::$controller->assign_title(__('店铺精确位置'));
