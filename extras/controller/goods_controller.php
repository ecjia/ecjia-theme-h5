...
class goods_controller
{
    ...
    public static function init()
    {
        ...
        if (!ecjia_front::$controller->is_cached('category_list.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign_title(__('所有分类'));
            ...
            if (!empty($goods_info['related_goods'])) {
                foreach ($goods_info['related_goods'] as $k => $v) {
                    if (!empty($v['specification'])) {
                        $spec_releated_goods[$v['goods_id']]['goods_price'] = ltrim((!empty($v['promote_price']) ? $v['promote_price'] : ($v['shop_price'] == __('免费') ? '0' : $v['shop_price'])), '￥');
    ...
    {
        ecjia_front::$controller->assign_title(__('促销商品'));
    ...
    {
        ecjia_front::$controller->assign_title(__('团购商品'));
    ...
    {
        ecjia_front::$controller->assign_title(__('新品首发'));
    ...
    {
        ecjia_front::$controller->assign_title(__('店长推荐'));
    ...
    public static function store_list()
    {
        ...
        if ($show_type == 'goods' && empty($store_id)) {
            ...
            ecjia_front::$controller->assign_title(__('商品列表'));
        ...
        ecjia_front::$controller->assign_title(__('店铺列表'));
