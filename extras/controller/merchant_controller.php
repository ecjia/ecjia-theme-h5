...
class merchant_controller
{
    ...
    public static function init()
    {
        ...
        if($open_type == 'affiliate') {
            ...
            //绑定店铺会员
            $result = RC_Api::api('customer', 'store_user_buy', array('user_id' => $_SESSION['user_id'], 'store_id' => $store_id, 'join_scene' => 'affiliate'));
            if (is_ecjia_error($result)) {
                return ecjia_front::$controller->showmessage($result->get_error_message(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url("merchant/index/init", ['store_id' => $store_id])));
            }
        }
