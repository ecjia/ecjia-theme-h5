...
//             'location' => array('longitude' => $_COOKIE['longitude'], 'latitude' => $_COOKIE['latitude']),
            ...
            if (!empty($goods_list)) {
                foreach ($goods_list as $k => $v) {
                    if (!empty($v['specification'])) {
                        $spec_goods[$v['id']]['goods_price']            = ltrim((!empty($v['promote_price']) ? $v['promote_price'] : ($v['shop_price'] == __('免费') ? '0' : $v['shop_price'])), '￥');
    ...
    public static function position()
    {
        ...
        if (!ecjia_front::$controller->is_cached('merchant_position.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign_title(__('店铺位置'));
        }
    ...
    public static function collectmoney()
    {
        ...
        ecjia_front::$controller->assign_title(__(sprintf("%s在线买单", $store_info['seller_name'])));
        ecjia_front::$controller->assign('title', __(sprintf("%s在线买单", $store_info['seller_name'])));
