...
class quickpay_controller
{
    ...
    public static function init()
    {
        ...
        if (empty($_SESSION['quick_pay']['data']['activity_list'])) {
            ...
            if (empty($store_info['allow_use_quickpay'])) {
                return ecjia_front::$controller->showmessage(__('该店铺未开启优惠买单活动'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR, array('pjaxurl' => RC_Uri::url('merchant/index/init', array('store_id' => $store_id))));
            }
    ...
    public static function done()
    {
        ...
        if (empty($goods_amount)) {
            return ecjia_front::$controller->showmessage(__('消费金额不能为空'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    public static function pay()
    {
        ...
        if (!ecjia_front::$controller->is_cached('quickpay_pay.dwt', $cache_id)) {
            ...
            if ($order_info['order_status_str'] == 'paid') {
                return ecjia_front::$controller->showmessage(__('该订单已支付请勿重复支付'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR);
            }
    ...
    public static function cancel()
    {
        ...
        if (!is_ecjia_error($data)) {
            return ecjia_front::$controller->showmessage(__('取消成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $url, 'is_show' => false));
        } else {
    ...
    public static function delete()
    {
        ...
        if (!is_ecjia_error($data)) {
            return ecjia_front::$controller->showmessage(__('删除成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $url));
        } else {
            return ecjia_front::$controller->showmessage($data->get_error_message(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $url));
        }
    }
    ...
    public static function notify()
    {
        ...
        ecjia_front::$controller->assign_title(__('支付成功'));
    ...
    public static function quickpay_list()
    {
        ...
        ecjia_front::$controller->assign_title(__('我的买单'));
    ...
    public static function quickpay_detail()
    {
        ...
        if (!is_ecjia_error($data)) {
            ...
            if ($data['order_status_str'] == '') {
                $url = RC_Uri::url('user/quickpay/quickpay_list');
                return ecjia_front::$controller->showmessage(__('该订单不存在'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $url));
            }
            ecjia_front::$controller->assign('data', $data);
        }

        ecjia_front::$controller->assign_title(__('买单详情'));
