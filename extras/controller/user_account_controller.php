...
class user_account_controller
{
    ...
    public static function init()
    {
        ...
        if (!ecjia_front::$controller->is_cached('user_account_detail.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign_title(__('我的钱包'));
        }
    ...
    public static function balance()
    {
        ...
        ecjia_front::$controller->assign_title(__('账户余额'));
        ecjia_front::$controller->assign('user', $user);
    ...
    public static function recharge()
    {
        ...
        if (!ecjia_front::$controller->is_cached('user_account_recharge.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign('user', $user);
            ecjia_front::$controller->assign_title(__('充值'));
            //生成返回url cookie
    ...
    public static function withdraw()
    {
        ...
        ecjia_front::$controller->assign_title(__('提现'));
    ...
    public static function withdraw_account()
    {
        ...
        if ($withdraw_way == 'wechat') {
            ...
            if (!$result) {
                return ecjia_front::$controller->showmessage(__('您还未设置微信提现的真实姓名'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('url' => RC_Uri::url('user/profile/account_bind', array('type' => 'wechat'))));
            }
        }
        ...
        if (is_ecjia_error($user)) {
            return ecjia_front::$controller->showmessage(__('用户信息不存在'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if ($amount < $min_withdraw_amount) {
            return ecjia_front::$controller->showmessage(__(sprintf("最低提现金额不能小于%s", $config['formatted_min_withdraw_amount'])), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    {
        ecjia_front::$controller->assign_title(__('交易记录'));
    ...
    public static function ajax_record()
    {
        ...
        if (!is_ecjia_error($account_list)) {
            ...
            foreach ($data as $key => $val) {
                ...
                if ($day == $now_day) {
                    $arr[$time][$key]['add_time'] = __(sprintf("今天%s", substr($val['add_time'], 11, 5)));
                } else {
    ...
    public static function record_info()
    {
        ...
        if (!ecjia_front::$controller->is_cached('user_record_info.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign_title(__('交易明细'));
    ...
    public static function record_cancel()
    {
        ...
        if ($submit == __('取消')) {
            ...
            return ecjia_front::$controller->showmessage(__('取消成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('user/account/record')));
        } elseif ($submit == __('充值')) {
    ...
    public static function recharge_again()
    {
        ...
        if (!ecjia_front::$controller->is_cached('user_account_recharge_again.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign('user', $user);
            ecjia_front::$controller->assign_title(__('继续充值'));
        }
