...
class user_address_controller
{
    ...
    public static function address_list()
    {
        ...
        if ($type == 'choose') {
            ...
            ecjia_front::$controller->assign_title(__('选择收货地址'));
            ...
            ecjia_front::$controller->assign('address_list', $address_list);
            ecjia_front::$controller->assign_title(__('收货地址管理'));
    ...
    public static function add_address()
    {
        ...
        ecjia_front::$controller->assign_title(__('添加收货地址'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function insert_address()
    {
        if (empty($_POST['province']) || empty($_POST['city']) || empty($_POST['district']) || empty($_POST['street']) || empty($_POST['address']) || empty($_POST['consignee']) || empty($_POST['mobile'])) {
            return ecjia_front::$controller->showmessage(__('请完整填写相关信息'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
        }
        ...
        return ecjia_front::$controller->showmessage(__('添加地址成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $pjax_url));
    }
    ...
    public static function edit_address()
    {
        ...
        if (empty($id)) {
            return ecjia_front::$controller->showmessage(__('参数错误'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => ''));
        }
        $temp_key  = 'edit_' . $id;
        ...
        ecjia_front::$controller->assign_title(__('编辑收货地址'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function update_address()
    {
        ...
        unset($_SESSION['address']);
        return ecjia_front::$controller->showmessage(__('编辑地址成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $pjaxurl));
    }
    ...
    public static function del_address()
    {
        ...
        if (!$id) {
            return ecjia_front::$controller->showmessage(__('参数错误'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        }
        ...
        if ($address_info['default_address'] == 0) {
            ...
            if (is_ecjia_error($data)) {
                ...
                return ecjia_front::$controller->showmessage(__('删除成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $pjax_url));
            }
        } else {
            return ecjia_front::$controller->showmessage(__('该地址为默认的收货地址，不能删除'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        }
    }
    ...
    public static function set_default()
    {
        ...
        if (!is_ecjia_error($data)) {
            return ecjia_front::$controller->showmessage(__('设置成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => $pjax_url));
        } else {
            return ecjia_front::$controller->showmessage($data->get_error_message(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('pjaxurl' => $pjax_url));
        }
    }
    ...
    public static function near_location()
    {
        ...
        if (!empty($_GET['address_id'])) {
            ...
            if (empty($temp_data['tem_city']) && empty($_COOKIE['city_name'])) {
                return ecjia_front::$controller->showmessage(__('请先选择城市'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => ''));
            }
        }
        ecjia_front::$controller->assign('temp', $temp_data);
        ecjia_front::$controller->assign_lang();
        ecjia_front::$controller->assign_title(__('选择位置'));
