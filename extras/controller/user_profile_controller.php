...
class user_profile_controller
{
    ...
    public static function init()
    {
        ...
        ecjia_front::$controller->assign_title(__('个人资料'));
    ...
    public static function modify_username()
    {
        ...
        if (!ecjia_front::$controller->is_cached('user_modify_username.dwt', $cache_id)) {
            ...
            ecjia_front::$controller->assign('user', $user);
            ecjia_front::$controller->assign_lang();
            ecjia_front::$controller->assign_title(__('修改用户名'));
        }
    ...
    public static function modify_username_account()
    {
        ...
        if (strlen($name) > 20 || strlen($name) < 4 || !preg_match('/^[A-Za-z0-9_\-\x{4e00}-\x{9fa5}]+$/u', $name)) {
            return ecjia_front::$controller->showmessage(__('修改失败，请输入正确的用户名格式'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (!empty($name)) {
            $data = ecjia_touch_manager::make()->api(ecjia_touch_api::USER_UPDATE)->data(array('token' => $token, 'user_name' => $name))->run();
            if (!is_ecjia_error($data)) {
                return ecjia_front::$controller->showmessage(__('修改成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('user/profile/init')));
            } else {
    ...
    public static function edit_password()
    {
        ...
        if (empty($user_info['mobile_phone'])) {
            return ecjia_front::$controller->showmessage(__('请先绑定手机号码'), ecjia::MSGTYPE_ALERT | ecjia::MSGSTAT_ERROR);
        }
        ecjia_front::$controller->assign('mobile', $user_info['mobile_phone']);
        ecjia_front::$controller->assign_title(__('修改密码'));
        ecjia_front::$controller->assign('title', __('修改密码'));
        ecjia_front::$controller->assign_lang();
    ...
    public static function withdraw()
    {
        ...
        ecjia_front::$controller->assign_title(__('提现账户'));
    ...
    public static function get_code()
    {
        ...
        if (!empty($mobile)) {
            if ($user['mobile_phone'] == $mobile) {
                return ecjia_front::$controller->showmessage(__('该手机号与当前绑定的手机号相同'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            ...
            if ($user['email'] == $email) {
                return ecjia_front::$controller->showmessage(__('该邮箱地址与当前绑定的邮箱地址相同'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
    ...
    public static function modify_password()
    {
        ...
        if (empty($code)) {
            return ecjia_front::$controller->showmessage(__('请输入验证码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (empty($password)) {
            return ecjia_front::$controller->showmessage(__('请设置密码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        if (strlen($password) < 6) {
            return ecjia_front::$controller->showmessage(__('登录密码不能少于 6 个字符'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if (is_ecjia_error($data)) {
            ...
            return ecjia_front::$controller->showmessage(__('修改成功，请重新登录'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('user/privilege/pass_login')));
        }
    }
    ...
    public static function unbind_check_mobile()
    {
        ...
        ecjia_front::$controller->assign_title(__('验证手机号'));
    ...
    public static function unbind_withdraw()
    {
        ...
        if ($type == 'wechat' || $type == 'bank') {
            ...
            if (empty($id)) {
                return ecjia_front::$controller->showmessage(__('该提现方式不存在'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }

            if (empty($smscode)) {
                return ecjia_front::$controller->showmessage(__('请输入手机验证码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
    ...
    public static function unbind_wechat()
    {
        ...
        return ecjia_front::$controller->showmessage(__('解绑成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('touch/my/init')));
    }
    ...
    public static function bind_withdraw()
    {
        ...
        if ($type == 'bank') {
            ...
            if (empty($card_name)) {
                return ecjia_front::$controller->showmessage(__('请输入持卡人姓名'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if (empty($bank_en_short)) {
                return ecjia_front::$controller->showmessage(__('请选择所属银行'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if (empty($bank_name)) {
                return ecjia_front::$controller->showmessage(__('请输入开户行'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if (empty($bank_number)) {
                return ecjia_front::$controller->showmessage(__('请输入银行卡号'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            if (empty($smscode)) {
                return ecjia_front::$controller->showmessage(__('请输入手机验证码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            ...
            return ecjia_front::$controller->showmessage(__('绑定成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('user/profile/account_bind', array('type' => 'bank'))));
        }
        ...
        if ($type == 'wechat') {
            ...
            if (empty($real_name)) {
                return ecjia_front::$controller->showmessage(__('请输入微信认证的真实姓名'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
            ...
            return ecjia_front::$controller->showmessage(__('绑定成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('url' => RC_Uri::url('user/profile/account_bind', array('type' => 'wechat'))));
        }
    ...
    public static function set_pay_password()
    {
        ...
        ecjia_front::$controller->assign_title(__('支付密码'));
    ...
    public static function check_pay_pass()
    {
        ...
        if (empty($type)) {
            ...
            if ($password === $confirm_password) {
                ...
                return ecjia_front::$controller->showmessage(__('设置成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('touch/my/init')));
            } else {
                return ecjia_front::$controller->showmessage(__('两次密码输入不一致，请重试'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR, array('type' => 'alert'));
            }
        }
    }
    ...
    public static function check_mobile()
    {
        ...
        $user_info = ecjia_touch_user::singleton()->getUserinfo();
        if (empty($user_info['mobile_phone'])) {
            return ecjia_front::$controller->showmessage(__('请先绑定手机号码'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
    ...
    public static function confirm_activate_account()
    {
        ...
        if (is_ecjia_error($data)) {
            ...
            return ecjia_front::$controller->showmessage(__('激活成功'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('touch/my/init')));
        }
    }
