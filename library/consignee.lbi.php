...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<div class="form-group">
		<label class="input">
			<input name="consignee" placeholder='{t}收货人姓名必填{/t}' type="text" value="{$consignee.consignee|escape}" datatype="s1-15" errormsg='{t}请输入长度为3 ~ 15的收货人姓名{/t}' />
		</label>
	</div>
	<div class="form-group">
		<label class="input">
			<input name="mobile" placeholder='{t}手机必填{/t}' type="text" value="{$consignee.mobile|escape}" datatype="m" errormsg='{t}请输入正确格式的联系方式{/t}' />
		</label>
	</div>
	...
	<div class="form-group">
		<div class="ecjiaf-fl">
			<select class="form-select" name="country" id="selCountries" data-toggle="region_change" data-url="index.php?m=user&c=user_address&a=region" data-type="1" data-target="selProvinces" datatype="*">
				<option value="0">{t}请选择{/t}{$name_of_region[0]}</option>
		...
		<div class="ecjiaf-fl"> 
			<select class="form-select" name="province" id="selProvinces" data-toggle="region_change" data-url="index.php?m=user&c=user_address&a=region" data-type="2" data-target="selCities" datatype="*">
				<option value="0">{t}请选择{/t}{$name_of_region[1]}</option>
		...
		<div class="ecjiaf-fl">
			<select class="form-select" name="city" id="selCities" data-toggle="region_change" data-url="index.php?m=user&c=user_address&a=region" data-type="3" data-target="selDistricts" datatype="*">
				<option value="0">{t}请选择{/t}{$name_of_region[2]}</option>
		...
		<div class="ecjiaf-fl">
			<select class="form-select" name="district" id="selDistricts" datatype="*">
				<option value="0">{t}请选择{/t}{$name_of_region[3]}</option>
				{$consignee.district}
	...
	<!-- 购物车中存在实体商品显示详细地址 -->
		<div class="form-group">
			<label class="textarea">
				<textarea name="address"  id="address" placeholder='{t}详细地址{/t}' datatype="*" >{$consignee.address|escape}</textarea>
			</label>
		</div>
<!-- {/if} -->
<div class="ecjia-margin-t btn-consignee {if $smarty.session.user_id gt 0 and $consignee.address_id gt 0} two-btn {/if}">
	...
	<!-- 如果登录了，显示删除按钮 --> 
	<input  class="btn btn-info"  onclick="if (confirm('您确定要删除该收货人信息吗？')) location.href='{url path='flow/drop_consignee' args="id={$consignee.address_id}"}'" value='{t}删除{/t}'>
	<!-- {/if} -->
	<input class="btn btn-info" name="Submit" type="submit" value='{t}配送至这个地址{/t}'>
</div>
