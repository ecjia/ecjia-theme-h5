...
<div class="ecjia-normal-modal {$type}">
	...
	<div class="ecjia-normal-modal-content">
		<div class="title">{t}恭喜您！{/t}</div>
		<div class="title">{t}红包添加成功！{/t}</div>
		<div class="">
			<img src="{$theme_url}images/wallet/bonus_success.png" />
		</div>
		<a class="btn close-normal-btn success" href="javascript:;">{t}确认{/t}</a>	
	</div>
	{else if $type eq 'error'}
	<div class="ecjia-normal-modal-content">
		<div class="title">{t}您输入的号码有误，{/t}</div>
		<div class="title">{t}请核对后重新输入！{/t}</div>
		<div class="">
			<img src="{$theme_url}images/wallet/bonus_error.png" />
		</div>
		<a class="btn close-normal-btn" href="javascript:;">{t}重新输入{/t}</a>	
	</div>
	{else if $type eq 'bonus_info'}
	<div class="ecjia-normal-modal-content">
		<div class="bonus_title">{t}红包详情{/t}</div>
		<div class="bonus_item">
			<div class="left">{t}红包名称：{/t}</div>
			<div class="right">{$bonus_info.bonus_name}</div>
		</div>
		<div class="bonus_item">
			<div class="left">{t}红包金额：{/t}</div>
			<div class="right red_color">{$bonus_info.formatted_bonus_amount}</div>
		</div>
		<div class="bonus_item">
			<div class="left">{t}使用期限：{/t}</div>
			<div class="right red_color">{$bonus_info.formatted_start_date}-{$bonus_info.formatted_end_date}</div>
		</div>
		<div class="bonus_item new_line">
			<div class="left">{t}使用条件：{/t}</div>
			<div class="right">{$bonus_info.label_request_amount}</div>
		</div>
		<a class="btn confirm-add-btn nopjax external" href="javascript:;" data-href="{RC_Uri::url('user/bonus/add_bonus')}">{t}确认添加{/t}</a>
		<a class="btn close-normal-btn gray" href="javascript:;">{t}取消添加{/t}</a>
	</div>
	{/if}
</div>
