...
<div class="ecjia-goodsAttr-modal ecjia-attr-modal">
	...
	<div class="modal-buttons modal-buttons-2 modal-buttons-vertical">
		...
		<a class="add-tocart add_spec {if $current_spec.rec_id}hide{else}show{/if}" data-toggle="add-to-cart" goods_id="{$goods_info.id}">{t domain="h5"}加入购物车{/t}</a>
...
<!--		<input type="hidden" name="product_id" data-url="{RC_Uri::url('goods/index/show')}&goods_id={$goods_info.goods_id}"  value="{if $current_spec.product_id}{$current_spec.product_id}{else}0{/if}" />-->
	</div>
</div>
