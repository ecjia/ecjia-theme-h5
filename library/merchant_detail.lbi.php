...
<div class="ecjia-store-detail ecjia-store-toggle hide" id="store-seller">
	...
	<ul class="store-promotion">
		<li class="quick">
			<span class="quick-label">{t}买单{/t}</span>
			<span class="quick-name">{t}买单立享优惠{/t}</span>
			<a class="quick-btn" href="{RC_Uri::url('user/quickpay/init')}&store_id={$store_info.id}" >{t}优惠买单{/t}</a>
		</li>
	...
	<div class="store-hr"></div>
	<div class="store-tel">
		<span class="tel-name"><i class="icon-shop-phone"></i>{t}商家电话{/t}</span>
		<p class="tel-result">{if $store_info.telephone}{$store_info.telephone}<a class="external" href="tel:{$store_info.telephone}"><i class="icon-call-phone"></i></a>{else}{t}暂无{/t}{/if}</p>
	</div>
	<div class="store-hr"></div>
	<ul class="store-other-info">
		<li>
			<span class="other-info-name"><i class="icon-shop-buliding"></i>{t}公司名称{/t}</span>
			<p class="other-info-result">{if $store_info.shop_name}{$store_info.shop_name}{else}暂无{/if}</p>
		</li>
		<li>
			<span class="other-info-name"><i class="icon-shop-location"></i>{t}所在地区{/t}</span>
			{if $store_info.shop_address}<a href="{$header_right.location_url}" class="nopjax external">{/if}
				<p class="other-info-result {if $store_info.shop_address}shop-address-result{/if}">{if $store_info.shop_address}{$store_info.shop_address}{else}{t}暂无{/t}{/if}</p>
				<i class="icon-position"></i>
			{if $store_info.shop_address}</a>{/if}
		</li>
		<li>
			<span class="other-info-name"><i class="icon-shop-time"></i>{t}营业时间{/t}</span>
			<p class="other-info-result">{if $store_info.label_trade_time}{$store_info.label_trade_time}{else}{t}暂无{/t}{/if}</p>
		</li>
		<li>
			<span class="other-info-name"><i class="icon-shop-description"></i>{t}商家简介{/t}</span>
			<p class="other-info-result">{if $store_info.seller_description}{$store_info.seller_description}{else}{t}暂无{/t}{/if}</p>
		</li>
		{if $store_info.business_licence_pic}
		<li>
			<span class="other-info-name"><i class="icon-shop-description"></i>{t}营业执照{/t}</span>
