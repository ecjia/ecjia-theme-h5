...
<li>
	<a class="linksGoods w nopjax external" href="{RC_Uri::url('goods/index/show')}&goods_id={$goods.id}&product_id={$goods.product_id}">
	...
	{if $shop_closed neq 1}
		...
		<div class="box" id="goods_{$goods.id}">
	    	<span class="reduce {if $goods.num}show{else}hide{/if}" data-toggle="remove-to-cart" rec_id="{$goods.rec_id}" goods_id="{$goods.id}" product_id="{$goods.product_id}">{t domain="h5"}减{/t}</span>
