...
<li>
	<a class="linksGoods w nopjax external" href="{RC_Uri::url('goods/index/show')}&goods_id={$goods.id}">
	...
	{if $shop_closed neq 1}
		...
		<div class="goods_attr goods_spec_{$goods.id}">
			<span class="choose_attr spec_goods" rec_id="{$goods.rec_id}" goods_id="{$goods.id}" data-num="{$goods.num}" data-spec="{$goods.default_spec}" data-url="{RC_Uri::url('cart/index/check_spec')}&store_id={$goods.store_id}">{t}选规格{/t}</span>
			{if $goods.num}<i class="attr-number">{$goods.num}</i>{/if}
		</div>
		{else}
		<div class="box" id="goods_{$goods.id}">
	    	<span class="reduce {if $goods.num}show{else}hide{/if}" data-toggle="remove-to-cart" rec_id="{$goods.rec_id}">{t}减{/t}</span>
	    	<label class="{if $goods.num}show{else}hide{/if}">{$goods.num}</label>
			<span class="add" data-toggle="add-to-cart" rec_id="{$goods.rec_id}" goods_id="{$goods.id}">{t}加{/t}</span>
		</div>
		{/if}
	{/if}
</li>
