...
<div class="ecjia-mod ecjia-header ecjia-store-banner" style="background: url('{if $store_info.seller_banner}{$store_info.seller_banner}{else}{$theme_url}images/default_store_banner.png{/if}') center center no-repeat;background-size: 144% 100%;">
	<div class="ecjia-store-brief">
			...
			<div class="basic-info">
				...
				<div class="store-right">
					<div class="store-title">
						...
						{if $store_info.manage_mode eq 'self'}<span class="manage-mode">{t}自营{/t}</span>{/if}
						{if $store_info.is_follower eq 1}
							<span class="follower not" data-toggle="follow_store" data-type=0 data-url="{$follow_url}">{t}已关注{/t}</span>
						{else}
							<span class="follower" data-toggle="follow_store" data-type=1 data-url="{$follow_url}"><i class="iconfont icon-add"></i>{t}关注{/t}</span>
						{/if}
					</div>
					<div class="store-range">
						<i class="icon-shop-time"></i>{$store_info.label_trade_time}
						{if $store_info.allow_use_quickpay eq 1}
						<a href="{RC_Uri::url('user/quickpay/init')}&store_id={$store_id}"><span class="check">{t}买单{/t}</span></a>
						{/if}
					</div>
		...
		<li class="favourable_notice">{t 1={$store_info.favourable_count}}共 %1 个活动{/t}<i class="iconfont icon-jiantou-right"></i></li>
		{/if}
	</div>
