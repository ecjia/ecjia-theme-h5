...
<div class="ecjia-search-panel">
	<header class="ecjia-header">
		<div class="ecjia-search-header ecjia-search">
			<form class="ecjia-form" action="{url path='goods/category/store_list'}{if $store_id neq 0}&store_id={$store_id}{/if}" name="search-form">
				<input id="keywordBox" name="keywords" type="search" placeholder='{if $store_id neq 0}{t}搜索店内商品{/t}{else}{t}搜索附近门店{/t}{/if}' {if $keywords}value={$keywords}{/if}>
				<i class="iconfont icon-search btn-search"></i>
			</form>
			<div class="search-cancel"><a href="javascript:;">{t}取消{/t}</a></div>
		</div>
	</header>
	...
	<div class="ecjia-search-history">
		<p class="title">
			{t 1={$searchs_count}}搜索（%1 条记录）{/t}{if $searchs}<a class="ecjiaf-csp" data-toggle="del_history" data-href="{url path='touch/index/del_search'}{if $store_id}&store_id={$store_id}{/if}"><i class="iconfont icon-delete"></i>{t}清除{/t}</a>{/if}
		</p>
		<div>
			<ul>
	...
	<!-- {else} -->
	<div class="ecjia-no-record">{t}您还没有搜索记录{/t}</div>
	<!-- {/if} -->
</div>
