...
<li class="single_item">
        ...
        <li class="store-info">
            <a class="nopjax external" href="{RC_Uri::url('merchant/index/init')}&store_id={$val.id}">
	            <div class="basic-info">
	                <div class="store-left">
	                    ...
	                    {if $val.shop_closed eq 1}
							<div class="shop_closed_mask">{t}休息中{/t}</div>
						{/if}
	                </div>
	                <div class="store-right">
	                    <div class="store-title">
	                    <span class="store-name">{$val.seller_name}</span>
	                    {if $val.manage_mode eq 'self'}<span class="manage_mode">{t}自营{/t}</span>{/if}</div>
	                    ...
	                    <div class="store-range">
							...
							<a href="{RC_Uri::url('user/quickpay/init')}&store_id={$val.id}"><span class="store-quickpay-btn">{t}买单{/t}</span></a>
							<!-- {/if} -->
	                    </div>
	                </div>
	                <div class="clear"></div>
				...
				<ul class="store-promotion">
					...
					{if $key eq 0}
					<li class="promotion">
						<span class="promotion-label">{t}买单{/t}</span>
    ...
    <ul>
        <li class="goods-info view-more">
			{t 1={$val.goods_count-3}}查看更多（%1）{/t}<i class="iconfont icon-jiantou-bottom"></i>
        </li>
        <li class="goods-info view-more retract hide">
			{t}收起{/t}<i class="iconfont icon-jiantou-top"></i>
        </li>
    </ul>
    {/if}
</li>		
