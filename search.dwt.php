...
<div class="ecjia-search-history">
	<p class="title">
		<i class="iconfont icon-time"></i>{t}搜索历史{/t}{if $searchs}<a class="ecjiaf-csp" data-toggle="del_history" data-href="{url path='touch/index/del_search'}{if $store_id}&store_id={$store_id}{/if}">{t}清除记录{/t}</a>{/if}
	</p>
	<div>
		<ul>
			<!-- {foreach from=$searchs item=search} -->
			<a href='{url path="goods/category/store_list" args="{if $store_id neq 0}store_id={$store_id}&{/if}keywords={$search}"}'><li>{$search}</li></a>
			<!-- {/foreach} -->
		</ul>
	</div>
</div>
