...
<div class="location-city">
    ...
    <ul class="location-city-content citylist">
        <li data-id="{$smarty.cookies.position_city_id}"
            class="{if $smarty.cookies.position_city_id eq $smarty.cookies.city_id}active{/if}">
            {$smarty.cookies.position_city_name}
        </li>
    </ul>
</div>
{/if}
{if $smarty.get.type eq 'addcity'}
<div class="cityall" id="cityall"
     data-url='{url path="user/address/add_address" args="{if $referer_url}&referer_url={$referer_url|escape:"url"}{/if}"}'>
        ...
        <div class="cityall" id="cityall" data-url="{url path='location/index/select_location'}">
            ...
            <div class="cityall" id="cityall" data-url="{url path='location/index/select_location'}">
                ...
                <ul class="city citylist">
                    ...
                    <li data-id="{$list.id}" {if $list.id eq $smarty.cookies.city_id}class="active" {
                    /if}>{$list.name}</li>
                    <!-- {foreachelse} -->
                    <li>暂无</li>
                    <!-- {/foreach} -->
