...
<form class="ecjia-address-list" name="theForm" action="{$form_action}" data-save-url="{url path='user/address/save_temp_data'}" method="post">
	<div class="form-group form-group-text ecjia-border-t">
		...
		<label class="input">
			<span>所在地区： </span>
			<input name="city_name" placeholder="{t}请选择城市{/t}" type="text" ignore="ignore" datatype="*" value="{if $temp.tem_city_name && $temp.tem_city gt 0}{$temp.tem_city_name}{else}{if $info.district_name}{$info.district_name}{else}{$smarty.cookies.city_name}{/if}{/if}" nullmsg="请选择城市" readonly="readonly" />
