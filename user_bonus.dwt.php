...
<ul class="ecjia-list ecjia-list-three ecjia-bonus ecjia-nav ecjia-bonus-border-right">
	...
	<li {if $smarty.get.status eq 'is_used'} class="red-bottom"{else}class=""{/if}><a {if $smarty.get.status eq 'expired'} class="red-font"{else}class=""{/if} href="{url path='user/user_bonus/bonus' args='status=is_used'}">{t}已使用{/t}</a></li>
	<li {if $smarty.get.status eq 'expired'} class="red-bottom"{else}class=""{/if}><a {if $smarty.get.status eq 'is_used'} class="red-font right-border"{else}class="right-border"{/if} href="{url path='user/user_bonus/bonus' args='status=expired'}">{t}已过期{/t}</a></li>
</ul>
<div>
