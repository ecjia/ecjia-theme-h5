...
<form class="ecjia-user ecjia-login-user-profile-form ecjia-login-padding-top" name="user_profile">
	<div class="ecjia-form">
    	...
    	<div class="ecjia-login-b ecjia-login-padding-top2">
    	    <div class="p-top3">
                <input type="hidden" name="referer" value="{$smarty.get.referer}" />
                <input name="modify_username" {if $limit_time}type="reset"{else}type="submit"{/if} class="btn btn-info nopjax {if $limit_time}not_submit{/if}" data-url="{url path='user/user_profile/modify_username_account'}" value="确定" />
    	    </div>	
    	</div>
	</div>
</form>
