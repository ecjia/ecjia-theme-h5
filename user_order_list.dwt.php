...
<li class="ecjia-order-item ecjia-checkout ecjia-margin-t {if $type == "whole"}ecjia-order-mt{/if}">
	<div class="order-hd">
		<a class="ecjiaf-fl" href='{url path="merchant/index/init" args="store_id={$list.seller_id}"}'>
			<span class="order_model">{if $list.order_mode eq 'default'}【配送】{elseif $list.order_mode eq 'storepickup'}【自提】{elseif $list.order_mode eq 'storebuy'}【到店购物】{/if}</span>{$list.seller_name}<i class="iconfont icon-jiantou-right"></i>
		</a>
