...
<li class="ecjia-order-item ecjia-checkout ecjia-margin-t {if $type == "whole"}ecjia-order-mt{/if}">
	<div class="order-hd">
		...
		<a class="ecjiaf-fr" href='{url path="user/order/order_detail" args="order_id={$list.order_id}&refund_sn={$list.refund_sn}"}'><span class="ecjia-color-green">{$list.label_service_status}</span></a>
	</div>
	<div class="flow-goods-list">
		<a class="ecjiaf-db" href='{url path="user/order/return_detail" args="order_id={$list.order_id}&refund_sn={$list.refund_sn}&type=detail"}'>
	...
	<div class="order-ft">
		<span>{$list.formated_add_time}</span>
		<span class="two-btn ecjiaf-fr">
			{if $list.service_status_code eq 'refunded'} 
				<a class="btn btn-hollow" href='{url path="user/order/return_detail" args="order_id={$list.order_id}&refund_sn={$list.refund_sn}&type=return_money"}'>查看退款</a>
			{/if}
