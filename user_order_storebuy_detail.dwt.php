	...
	<div class="ecjia-checkout ecjia-margin-b">
		<div class="flow-goods-list">
			...
			<div class="order-ft-link">
				...
				{if $order.pay_status eq 0 && $order.order_status lt 2}<a class="btn btn-small btn-hollow cancel_order_unpay" href='{url path="user/order/order_cancel" args="order_id={$order.order_id}"}'>{t domain="h5"}取消订单{/t}</a> <a class="btn btn-small btn-hollow" href='{url path="payment/pay/init" args="order_id={$order.order_id}"}'>{t domain="h5"}去支付{/t}</a>{/if}
