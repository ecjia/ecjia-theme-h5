        ...
        {if $sur_amount.type eq 'deposit'}
            <div class="two-btn btn-bottom">
        		...
        		{if $recharge_action eq 'again'}
        			<a  class="btn ecjiaf-fr ecjia-fl nopjax"href="{RC_Uri::url('user/account/recharge_again')}&format_amount={$sur_amount.format_amount}&order_sn={$sur_amount.order_sn}&account_id={$sur_amount.account_id}" >{t}充值{/t}</a>
        		{else}
	        		...
	        		{else}
	        			<input class="btn btn-recharge ecjiaf-fr ecjia-fl alipay-btn" type="button" value="{t}充值{/t}" />
