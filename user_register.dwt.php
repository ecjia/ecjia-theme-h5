...
<div class="user-register">
    ...
    <!-- {/if} -->
        <div class="tab-pane{if $enabled_sms_signin neq 1} active{/if}" id="two">
    		<form class="ecjia-form ecjia-login ecjia-login-margin-top" name="formUser" action="{url path='user/index/set_password'}" method="post">
    			...
    			<div class="form-group small-text">
    				<label class="input-1">
    					<input name="code" type="code" id="code" datatype="e" placeholder="{$lang.input_verification}" />
    				</label>
    			</div>
    			...
    			<div class="ecjia-login-b">
    				...
    				<a href="{url path='user/index/set_password'}">
    				<button class="btn btn-info login-btn" name="Submit" type="submit">{$lang.next}</button>
    				</a>
    				</div>
    			</div>
    		</form>
